CREATE TABLE Users (
    ...
    date_of_birth TEXT NOT NULL,  -- Remove the CHECK constraint
...
CREATE TABLE PersonalityTestQuestions (
    ...
    question_type TEXT NOT NULL CHECK (question_type IN ('yes_no', 'multiple_choice')) -- Removed trailing comma
);
...
-- Create Temporary Personality Test Results Table
...
-- Trigger to clean up temporary results
...
-- Trigger to log answer submissions
...
BEGIN
    ...
    VALUES (NEW.user_id, 'ANSWER_SUBMISSION', 'Answer submitted for question ID ' || NEW.question_id);
END;
